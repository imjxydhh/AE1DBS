...
function displayRoom(){
	...
	var url = "staffviewall_ajax.php";
	var period = getPeriod();
	var startDate = period.startDate;
	var endDate = period.endDate;
	var roomType = infoForm.elements[6].value;
	var floor = document.getElementById("currentPage").innerHTML * 1.0;
	//url = url + "&sid=" + Math.random();
	xmlHttp.onreadystatechange = stateChanged;
	xmlHttp.open("POST", url, true);
	xmlHttp.setRequestHeader('content-type','application/x-www-form-urlencoded');
	xmlHttp.send("startDate="+startDate+"&endDate="+endDate+"&roomType="+roomType+"&floor="+floor);
	return false;
}
